class Bot
  attr_accessor :factory, :id, :chips, :low_to_type, :low_to_id, :high_to_type, :high_to_id

  def initialize(factory, id, low_to_type, low_to_id, high_to_type, high_to_id)
    self.factory      = factory
    self.id           = id
    self.chips        = []
    self.low_to_type  = low_to_type
    self.low_to_id    = low_to_id
    self.high_to_type = high_to_type
    self.high_to_id   = high_to_id
  end

  def collect_chip(chip_id)
    self.chips << chip_id
    chips.sort!
    deliver_chips if factory.report(id, chips)
  end

  private

  def deliver_chips
    factory.make_transaction(chips.shift, low_to_type, low_to_id)
    factory.make_transaction(chips.shift, high_to_type, high_to_id)
  end
end

class Output
  attr_accessor :id, :chip
