...
MESSAGES.map { |m| m.split('') }.transpose.each do |m|
  message << count_letters(m).max_by { |h| h[:count] }[:letter]
end
...
MESSAGES.map { |m| m.split('') }.transpose.each do |m|
  message << count_letters(m).min_by { |h| h[:count] }[:letter]
end
